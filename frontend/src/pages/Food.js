...
function Food() {
  const [highProtein, setProtein] = useState(false);
  const [lowFat, setFat] = useState(false);
  const [lowCal, setCal] = useState(false);
  const [df, setdf] = useState(false);
  const [gf, setgf] = useState(false);
  const [veg, setVeg] = useState(false);
  const [recipes, setRecipes] = useState([]);

  setRecipes(searchRecipes(highProtein, lowFat, lowCal, df, gf, veg, recipes));
